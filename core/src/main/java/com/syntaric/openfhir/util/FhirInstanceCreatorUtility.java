...
public class FhirInstanceCreatorUtility {
    ...
    public Object handleSpecialThisKeyword(final Object generatedInstance, final boolean resolveFollows,
        ...
        final boolean isEnumeration =
                theField.getType().equals(Enumeration.class) && objectToReturn instanceof CodeType;
        ...
        try {
            ...
            if (theField.getType() == List.class) {
                ...
                if ("BundleEntryComponent".equals(theField.getDeclaringClass().getSimpleName())
                        && value instanceof Reference refValue) {
                    ((BundleEntryComponent) resource).setResource((Resource) refValue.getResource());
                } else {
    ...
    public Class<? extends IBaseResource> getFhirResourceType(final String resourceName) {
        try {
            return (Class<? extends IBaseResource>) Class.forName(
                    Patient.class.getPackage().getName() + "." + resourceName);
